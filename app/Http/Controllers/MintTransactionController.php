...
class MintTransactionController extends Controller
{
    ...
    {   
        ...
        foreach ($chain as [$addr, $lovelace, $value, $string]) {
            ...
            if (!isset($combinedMints[$addr])) {
                if (strcmp($string, 'ADA')) {                    
                    $combinedMints[$addr] = '--mint "'.$value.' '.$string.'"';
                }
            } else {
                if (strcmp($string, 'ADA')) {                    
                    $combinedMints[$addr] .= '--mint "'.$value.' '.$string.'"';
        ...
        foreach ($combinedMints as $addr => $combined) {
               $txout[] = $combined;
        }
        ...
        $mintingScript = $user->wallet->policy_script;

        // dd($mintingScript);

        try {
            $transactionFee = $this->cli->buildTransaction($txins, $txout, $address, 'matx.raw', 0, $nftMetaData, $mintingScript);
